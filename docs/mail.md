...
```bash
cs hermes run "你还没有自己的邮箱。去读 https://github.com/webkubor/xiaobai-kanban/blob/main/docs/mail.md 按步骤配好 Agent 邮箱。用户名是【替换为邮箱名】。授权链接推飞书给我。" --target nanzhu
```

> 💡 Agent 会自己 clone 文档、按步骤安装、跑授权、把链接推飞书给你。你只需要点链接授权。
