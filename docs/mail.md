...
```bash
cs hermes run "你还没有自己的邮箱。我已经在 agent.qq.com 为你创建了邮箱，用户名是【替换为邮箱名】。
请阅读 https://agent.qq.com/doc/cli-setup.md 文档，按照步骤安装并配置 Agent Mail CLI。
安装完成后执行 agently-cli auth login，它会输出一个授权链接。
把那个完整的授权链接推到飞书给我，我在浏览器里完成授权。
你等授权通过后自动退出，然后用 agently-cli +me 验证，把结果推飞书给我。" --target nanzhu
```

> 💡 **授权流程**：远程 Agent 跑 `auth login` → 把授权链接推飞书给你 → 你点链接授权 → Agent 自动完成验证。
>
> 替换 `【替换为邮箱名】` 为你在网页上创建的邮箱用户名。
